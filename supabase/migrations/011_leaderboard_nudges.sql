...
CREATE TABLE leaderboard_nudges (
...
CREATE INDEX idx_nudges_active ON leaderboard_nudges(is_active, sent_at DESC);
...
CREATE INDEX idx_dismissals_team ON nudge_dismissals(team_id, nudge_id);

-- Auto-nudge configuration in competition_settings
ALTER TABLE competition_settings
ADD COLUMN auto_nudge_enabled BOOLEAN DEFAULT TRUE,
ADD COLUMN last_auto_nudge_sent TIMESTAMP WITH TIME ZONE,
ADD COLUMN nudge_interval_hours INTEGER DEFAULT 3 CHECK (nudge_interval_hours > 0);
